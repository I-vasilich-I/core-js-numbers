...
function getParallelepipedDiagonal(a, b, c) {
  // I'd say more accurate is - Math.hypot(a, b, c);
...
function getSumOfNumbers(x1, x2, x3) {
  ...
  [x1, x2, x3].forEach((n) => {
    const float = n - Math.trunc(n);
    fractionDigits = Math.max(float.toString().length, fractionDigits);
  });

  return +(x1 + x2 + x3).toFixed(fractionDigits);
}
...
function getCountOfOddNumbers(number) {
  const count = Math.abs(number / 2);
  return number % 2 ? Math.floor(count) + 1 : count;
}
